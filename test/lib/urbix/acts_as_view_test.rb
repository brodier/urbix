...
class ActsAsViewTest < ActiveSupport::TestCase
  ...
  test "View Relations Class on Mail and Address model" do
    vr = Urbix::ActsAsView::ViewRelations.new(PostalMail)
    ...
    assert_equal ["addresses", "addresses addresses_1", "cities", "cities cities_1", "countries", 
      "countries countries_1", "localities", "localities localities_1", "postal_mails"].sort,vr.from_clause.split(', ').sort
    assert_equal ["addresses.locality_id=localities.id", "addresses_1.locality_id=localities_1.id", 
      ...
      "localities_1.city_id=cities_1.id", "localities_1.country_id=countries_1.id", "postal_mails.dst_id=addresses_1.id", 
      "postal_mails.exp_id=addresses.id"].sort, vr.join_clause.split(' and ').sort
  end
  
  test "Mail View method" do
    # view should return same record
    assert_equal PostalMail.all.collect{|m| m.content}.sort,PostalMail.view.collect{|m| m.content}.sort
    # Check view attributes
    assert_equal PostalMail.view.first.attributes.keys,  ["id", "content", "exp_id", 
    "dst_id", "created_at", "updated_at", "exp_city", "exp_country", "dst_city",
    "dst_country"]
    assert_equal PostalMail.view.collect{|m| 
      [m.exp_city,m.exp_country,m.dst_city,m.dst_country]
    }.sort , PostalMail.all.collect{|m| 
